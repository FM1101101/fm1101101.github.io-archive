! function (undefined) {
    ...
    function iFrameListener(event) {
        ...
        var msg = event.data,
            messageData = {},
            iframeId = null;
        "[iFrameResizerChild]Ready" === msg ? iFrameReadyMsgReceived() : isMessageForUs() ? (messageData = processMsg(), iframeId = logId = messageData.id, settings[iframeId].loaded = !0, !isMessageFromMetaParent() && hasSettings(iframeId) && (log(iframeId, "Received: " + msg), checkIFrameExists() && isMessageFromIFrame() && actionMsg())) : info(iframeId, "Ignored: " + msg)
    }
    ...
    function trigger(calleeMsg, msg, iframe, id, noResponseWarning) {
        ...
        function warnOnNoResponse() {
            ...
            noResponseWarning && settings[id].warningTimeout && (settings[id].msgTimeout = setTimeout(warning, settings[id].warningTimeout))
        }
        var errorShown = !1;
    ...
    function setupIFrame(iframe, options) {
        ...
        function setScrolling() {
            switch (log(iframeId, "IFrame scrolling " + (settings[iframeId].scrolling ? "enabled" : "disabled") + " for " + iframeId), iframe.style.overflow = !1 === settings[iframeId].scrolling ? "hidden" : "auto", settings[iframeId].scrolling) {
                case !0:
    ...
    function sendTriggerMsg(eventName, event, data) {
        ...
        for (var iframeId in settings) isIFrameResizeEnabled(iframeId) && trigger(eventName, event + (data ? ":" + data : ""), document.getElementById(iframeId), iframeId)
    }
